...
def stat(name:str, value, dollar:bool = True, comparison=None):
    ...
    return (
        ...
        Article(cls="relative p-6 col-span-1 group bg-base-100 hover:bg-base-200 transition-colors duration-200", id=f'stat-{name}')(
            ...
            Div(cls=f"mt-1 flex gap-1 text-{'success' if comparison >=0 else 'error'}")(
                SVG_MAP.get('increase') if comparison >= 0  else SVG_MAP.get('decrease'),
                P(cls="flex gap-2 text-xs")(Span(f"{pct_chng:,.2f}%", cls="font-medium")),
            )
        )
    )

def stat_chart(name, dates, timeseries, totals, x_label):
    c = embed_line_chart(dates, [i.get(name) for i in timeseries], x_axis_label=x_label)    
    return (
        Div(id=f'metric-container-{name}', cls="contents")(
        stat(name=name, value=totals.get('selected_period').get(name), dollar=name not in UNIT_FIELDS), 
...
def render_content(content: DashContent, hx_swap:str=None):
    ...
    data_selected = [dict(row) for row in data.get('selected_period')]
    # get dates
    dates = [i.get('date') for i in data_selected]
    stat_charts = [stat_chart(f, dates, data_selected, totals, x_label=True if n==0 else False) for n, f in enumerate(content.fields)]
...
## this needs to remove metric from fields
@rt("/remove-metric")
def post(field:str, fields:list[str]):
    but = Template()(Li(A(field.replace('_', ' ').title(), hx_swap_oob='outerHTML', hx_post='/append-metric', hx_swap='beforeend', hx_target='#stat-chart-content', hx_vals=dict(field=field),cls=f'm-1', id=f'metric-{field}-button', onclick="this.classList.toggle('menu-active');")))
...
def post(field:str, group:str, time:str, comparison:str, fields:list[str]):
    ...
    data_selected = [dict(row) for row in data.get('selected_period')]
    dates = [i.get('date') for i in data_selected]
    return (stat_chart(field, dates, data_selected, totals, x_label=False), Input(type='hidden', name='fields', value=fields, id='fields-value', hx_swap_oob='outerHTML'))

@rt("/")
def get():
    ## load defaults for user/session
    ## DashContent()
    content = DashContent() # for now
...
def post(content:DashContent, pressed:str):
    ...
    out = (_option(value=getattr(content, pressed), option_grp=pressed, anchor=OPTION_ANCHOR_MAP.get(pressed)), Input(type='hidden', name=pressed, value=getattr(content, pressed), id=f'{pressed}-value', hx_swap_oob='outerHTML'),)
